...
class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    func setupStatusButton() {
        if let statusButton = statusItem.button {
            let image = #imageLiteral(resourceName:"Status")
            image.isTemplate = true
            statusButton.image = image
            let highlightedImage = #imageLiteral(resourceName: "StatusHighlighted")
            highlightedImage.isTemplate = true
            statusButton.alternateImage = highlightedImage
    ...
    func openPopover() {
        if let statusButton = statusItem.button {
            ...
            popover.show(relativeTo: NSZeroRect, of: statusButton, preferredEdge: NSRectEdge.maxY)
